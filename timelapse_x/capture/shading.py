...
Manages temporary viewport settings during capture.
"""
...
def lock_viewport_shading(scene, prefs) -> Optional[str]:
    ...
    Temporarily changes viewport shading and stores original settings
    ...
    Returns:
        Original render engine or None if no lock applied
    """
    ...
    for window in wm.windows:
        ...
        for area in screen.areas:
            ...
            for space in area.spaces:
                ...
                state.backup_shading_settings(space, shading, overlay, show_gizmo)
...
def restore_viewport_shading(scene, original_engine: Optional[str]):
    ...
    Args:
        scene: Blender scene object
        original_engine: Original render engine to restore
    """
    # Restore engine
    ...
    backup = state.get_shading_backup()
...
def apply_workbench_display_overrides(
    ...
    Args:
        scene: Blender scene object
        want_xray: Enable X-Ray mode
        want_no_shadows: Disable shadows
...
def restore_workbench_display_overrides(scene, backup: Optional[dict]):
    ...
    Args:
        scene: Blender scene object
        backup: Backup dictionary from apply function
    """
    if not backup:
        return
...
def apply_low_overhead_settings(scene) -> dict:
    ...
    Reduces quality settings to minimum for speed:
    - Simplify enabled
    - Low samples
    - No denoising
    - No motion blur
...
def register():
    ...
    logger.info("Shading module registered")
