...
ENHANCED VERSION with Edit Mode Options and Object Color Support:
- SKIP: Skip capture in edit mode (safe, recommended)
- FORCE_OBJECT: Temporarily switch to Object mode (may interrupt)
- CAPTURE_ANYWAY: Capture in edit mode (risky, may crash)
- Object colors: White by default, or use viewport colors
"""
...
def _is_in_edit_mode() -> Tuple[bool, Optional[str]]:
    ...
    Returns:
        Tuple of (is_edit_mode, mode_name)
    """
    try:
...
def capture_cameras(context, base_dir: str, require_dirty: bool = True) -> bool:
    ...
    # Ignore depsgraph during capture
    state.TLX_State.ignore_depsgraph = True
    ...
    try:
        ...
        if any_captured:
            state.TLX_State.dirty = False
        
        return any_captured

    finally:
        # Restore depsgraph monitoring
        state.TLX_State.ignore_depsgraph = False
...
def register():
    ...
    logger.info("Camera capture module registered (Edit Mode Options + Object Colors)")
