...
Complete version with all features including wireframe customization.
"""
...
class TLX_PT_panel(Panel):
    ...
    def _draw_recording_progress(self, layout, context):
        ...
        if rec_progress.frame_count > 0:
            ...
            # Session folder link
            if state.TLX_State.session_dir and os.path.isdir(state.TLX_State.session_dir):
                col.separator()
        ...
        if state.TLX_State.paused:
            ...
            mode_name = state.TLX_State.capture_mode.replace('_', ' ').title()
    ...
    def _draw_session_info(self, layout, prefs):
        ...
        # Frame counter
        row = box.row()
        row.label(text=f'Frames: {state.TLX_State.counter}')
        
        # Recording status
        if state.TLX_State.recording:
            status = "PAUSED" if state.TLX_State.paused else "RECORDING"
            row.label(text=status, icon='REC' if not state.TLX_State.paused else 'PAUSE')
        
        # Last capture time
        if state.TLX_State.last_capture_time:
            try:
                time_str = datetime.fromtimestamp(
                    state.TLX_State.last_capture_time
                ).strftime("%H:%M:%S")
                box.label(text=f'Last capture: {time_str}')
            except:
                pass
        
        # Session folder stats
    ...
    def _draw_video_tools(self, layout, scene, prefs):
        ...
        if not (prefs and collapsed):
            ...
            session_active = bool(state.TLX_State.session_dir and 
                                os.path.isdir(state.TLX_State.session_dir))
...
def register():
    ...
    logger.info("Registering UI panels")
